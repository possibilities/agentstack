...
function packageSection(server: CatalogServer): string {
  ...
  const events = topics.length ? `<section class="events" id="events-${id(server.name)}"><h3>Change events</h3><p>Notices carry a topic name only; read the current state after subscribing or reconnecting.</p>${scope}<div class="event-list">${topics.map(([name, description]) => `<div class="event"><code>${escape(name)}</code><span>${escape(description)}</span></div>`).join("")}</div>${socketSubscription ? `<div class="subscription"><h4>Subscribe on the socket</h4><p>Send one JSON line to this package's socket. The acknowledgement returns the accepted topics${server.eventScope?.required ? " and scope" : ""}; subsequent notices use <code>events/changed</code>.</p><pre><code>${escape(JSON.stringify(request, null, 2))}</code></pre></div>` : ""}</section>` : "";
