...
function packageSection(server: CatalogServer): string {
  ...
  const request = { id: 1, method: "events/subscribe", params: { topics: topics.map(([name]) => name) } };
  const events = topics.length ? `<section class="events" id="events-${id(server.name)}"><h3>Change events</h3><p>Notices carry a topic name only; read the current state after subscribing or reconnecting.</p><div class="event-list">${topics.map(([name, description]) => `<div class="event"><code>${escape(name)}</code><span>${escape(description)}</span></div>`).join("")}</div>${socketSubscription ? `<div class="subscription"><h4>Subscribe on the socket</h4><p>Send one JSON line to this package's socket. The acknowledgement returns the accepted topics; subsequent notices use <code>events/changed</code>.</p><pre><code>${escape(JSON.stringify(request, null, 2))}</code></pre></div>` : ""}</section>` : "";
...
export function renderDocs(servers: CatalogServer[], revision: string): string {
  ...
  return `<!doctype html><html lang="en" data-revision="${escape(revision)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><title>Package API reference · AgentStack</title><link rel="stylesheet" href="/site.css"><script src="/site.js" defer></script></head><body>
    ...
    <main id="main"><header class="topline"><span>Developer reference</span><span id="source-status">Live from the api socket</span></header><div class="content"><div class="opening"><div><h1>Package API reference</h1><p class="lede">The local operations and change events that make up AgentStack. Browse the current contract, including fields, types, and socket paths.</p></div><div class="figures"><div><strong>${servers.length}</strong><span>Packages</span></div><div><strong>${operations}</strong><span>Operations</span></div><div><strong>${events}</strong><span>Events</span></div></div></div>
