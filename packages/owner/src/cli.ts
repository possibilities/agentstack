...
import { apiChild, authChild, rolesChild, workersChild, wikiChild, websocketChild } from "./children.js";
...
owner = startOwner([apiChild(), authChild(), rolesChild(), botsChild(mcp.port), workersChild(), wikiChild(), websocketChild(), inspectorChild(catalog.path, inspectorListenPort), uixChild(uixListenPort)], process.env, () => {
  statusSource.notify();
...
}, [["auth"], ["workers"], ["bots"], ["wiki"], ["roles"], ["api"]]);
