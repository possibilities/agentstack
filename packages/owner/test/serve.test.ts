...
test("serve owns its socket and four children, then shuts them down", { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(status.children.map((entry) => entry.name).sort(), ["api", "auth", "bots", "codex"]);
    ...
    assert.ok(!stderr.includes("http://") && !stderr.includes("https://"), stderr);
