...
import { apiChild, authChild, rolesChild, websocketChild } from "../src/children.js";
...
test("the owner starts the required socket children", () => {
  for (const child of [apiChild(), authChild(), rolesChild(), botsChild()]) {
  ...
  assert.deepEqual([apiChild(), authChild(), rolesChild(), botsChild()].map((child) => child.name), ["api", "auth", "roles", "bots"]);
