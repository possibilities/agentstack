...
export class LoginManager {
  ...
  constructor(private readonly store: AuthStore, private readonly command: { bin: string; args: string[] } = {
  ...
  private async launch(replace: string | null): Promise<LoginState> {
    ...
    child.stdout?.on("data", (chunk: Buffer) => {
      if (state.status !== "pending") return;
      // The device prompt is the only CLI output the API may surface; never expose other output or credentials.
    ...
    try {
      if (state.status === "pending" && code === 0) {
        ...
        if (state.status === "pending") {
          ...
          } else completedAccount = this.store.addAccount(auth).id;
        }
      ...
      if (completedAccount) this.onAccountsChange?.();
      this.onChange?.();
    }
  }
}
