...
test("one HTTP process exposes each configured Package API and forwards operations to socket owners", { timeout: 30_000 }, async () => {
  ...
  const seen: string[] = [];
  const sockets = await Promise.all(["auth", "bots", "roles", "owner", "usage", "workers", "wiki"].map((name) => serveSocket({
  ...
  try {
    assert.deepEqual(Object.keys(served.urls), ["auth", "bots", "owner", "roles", "usage", "wiki", "workers"]);
