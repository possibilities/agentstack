...
import { collectAccount, collectGrokBot, ObservationFailure } from "./collect.js";
import { observationError, snapshotSchema, type Provider, type Snapshot, type StoredMeasurement } from "./schema.js";

export type Registered = { id: string; provider: Provider; enabled: boolean; ready: boolean; removing: boolean };
type FetchAccount = (id: string, provider: Provider) => Promise<StoredMeasurement["usage"]>;
type FetchBot = () => Promise<Snapshot["grokBot"]["usage"]>;
type LoadAccounts = () => Promise<Registered[]>;
type Row = { id: string; provider: Provider; enabled: boolean; ready: boolean; measurement: StoredMeasurement; nextAttemptAtMs: number };
type Persisted = { schemaVersion: 1; accounts: Row[]; bot: StoredMeasurement };
...
export class UsageObserver {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      if (value.schemaVersion !== 1 || !Array.isArray(value.accounts) || value.accounts.length > 128) return;
      for (const row of value.accounts) {
        const candidate = snapshotSchema.shape.accounts.element.safeParse({ ...row.measurement,
          id: row.id, provider: row.provider, enabled: row.enabled, ready: row.ready, fresh: false });
        if (candidate.success && Number.isSafeInteger(row.nextAttemptAtMs)) this.rows.set(row.id, {
          id: candidate.data.id, provider: candidate.data.provider,
  ...
  snapshot(now = Date.now()): Snapshot {
    const accounts = [...this.rows.values()].map(({ id, provider, enabled, ready, measurement }) => ({
      id, provider, enabled, ready, ...measurement, fresh: this.inventoryError === null &&
        (provider === "codex" || ready) && fresh(measurement, now),
  ...
  private async save(): Promise<void> {
    ...
    try {
      const value: Persisted = { schemaVersion: 1, accounts: [...this.rows.values()], bot: this.bot };
  ...
  async cycle(now = Date.now()): Promise<void> {
    ...
    const wanted = new Set(inventory.filter((row) => !row.removing).map((row) => row.id));
    ...
    for (const account of inventory) {
      ...
      const previous = this.rows.get(account.id);
      const row: Row = previous?.provider === account.provider ? { ...previous, enabled: account.enabled, ready: account.ready,
        ...
        id: account.id, provider: account.provider, enabled: account.enabled, ready: account.ready,
      ...
      if ((account.provider !== "codex" && !account.ready) || now < row.nextAttemptAtMs) continue;
      try {
        const usage = await this.fetchAccount(row.id, row.provider);
