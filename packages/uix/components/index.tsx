...
import { indexData } from "@/lib/index-data";
import type { Snapshot } from "@/lib/stack/types";
import { StackProvider, useStack } from "./canvas/provider";

const connections = { packages: ["owner", "bots"], scopedBots: false } as const;

export function Index({ snapshot }: { snapshot: Snapshot }) {
  return <StackProvider snapshot={snapshot} connections={connections}><IndexContent /></StackProvider>;
}

function IndexContent() {
  const { owner: ownerResource, bots: botResource } = useStack();
  const { owner, bots, links, mcp, children } = indexData(ownerResource.data, botResource.data);
