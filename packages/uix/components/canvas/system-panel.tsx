...
export function SystemPanel({ target, visible, onClose }: { target: SystemTarget | null; visible: boolean; onClose(): void }) {
  ...
  // The UI and its API reference are already open here; only MCP Inspector is a separate surface.
  const surfaces = [
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    ...
    <div ref={body} data-scroll className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
      ...
      {surfaces.length ? <Group title="Surfaces" count={surfaces.length} open={false}>{surfaces.map(({ title, url }) => <div key={title} className="flex min-w-0 items-start gap-1"><a href={url} target="_blank" rel="noreferrer" className="flex min-w-0 flex-1 items-start gap-2 rounded-sm text-xs hover:underline focus-visible:outline-2 focus-visible:outline-ring"><span className="flex min-w-0 flex-1 flex-col gap-1"><span>{title}</span><code className="break-all text-muted-foreground">{url}</code></span><ArrowUpRightIcon aria-hidden className="size-3 shrink-0" /><span className="sr-only">opens in a new tab</span></a><CopyButton value={url} label={`${title} URL`} className="shrink-0 opacity-100" /></div>)}</Group> : null}
