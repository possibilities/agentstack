...
export function SystemPanel({ target, visible, onClose }: { target: SystemTarget | null; visible: boolean; onClose(): void }) {
  ...
  const surfaces = data ? [{ title: "Runtime index", url: data.indexUrl }, { title: "UIX", url: data.uixUrl }, { title: "MCP Inspector", url: data.inspectorUrl }].filter((s) => s.url && matches(s.title, s.url)) : [];
  useEffect(() => {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    ...
    <div ref={body} data-scroll className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
      {owner.error ? <p className="py-3 text-xs text-destructive" role="status">{owner.error}{data ? " · Last successful status retained." : ""}</p> : null}
      ...
      <Group title="Surfaces" count={surfaces.length} open={false}>{surfaces.map(({ title, url }) => <a key={title} href={url!} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-2 rounded-sm text-xs hover:underline focus-visible:outline-2 focus-visible:outline-ring"><span>{title}</span><ArrowUpRightIcon className="ml-auto size-3 shrink-0" /><span className="sr-only">opens in a new tab</span></a>)}</Group>
      <Group title="Activity" count={activity.length}><p className="text-xs text-muted-foreground">Change notices invalidate snapshots; they carry no record payload.</p><ol className="flex flex-col gap-1">{activity.map((event) => {
