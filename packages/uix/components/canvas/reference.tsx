...
function SchemaFields({ schema, path = "", depth = 0 }: { schema: JsonSchema; path?: string; depth?: number }) {
  if (depth > 12) return <p className="text-xs text-muted-foreground">More in the full schema.</p>;
...
function Package({ doc }: { doc: PackageDoc }) {
  ...
  return <article className="flex min-w-0 flex-col gap-6">
    ...
    <section className="flex min-w-0 flex-col gap-3"><h4 className="text-sm font-semibold">Events and subscriptions</h4><p className="text-xs leading-relaxed text-muted-foreground">Notices carry a topic only; re-read state after (re)subscribing. Counts are this session’s.</p>
      ...
      {doc.transports.some((t) => t.type === "mcp" && t.supported) ? <p className="text-xs leading-relaxed text-muted-foreground">MCP has no subscribe frames. Verified Bot threads get generated event tools instead.</p> : null}
    </section><Separator />
