...
try {
  ...
  await page.addInitScript(() => {
    localStorage.setItem("agentstack.uix.canvas.v3", JSON.stringify({ spaces: { fleet: { mode: "grid" } } }));
  ...
  for (const theme of ["light", "dark"]) {
    ...
    for (const provider of providers) await card(provider).screenshot({ path: join(evidence, `${provider}-${theme}.png`), animations: "disabled" });
  }
  ...
  for (const theme of ["light", "dark"]) {
    ...
    for (const provider of providers) {
      ...
      assert.ok(bounds.x >= 0 && bounds.x + bounds.width <= 391, `${provider} fits narrow viewport`);
      assert.equal(await panel.evaluate((element) => element.scrollWidth <= element.clientWidth), true, `${provider} content does not overflow`);
