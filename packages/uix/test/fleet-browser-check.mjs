...
try {
  ...
  await dialog.getByRole("radio", { name: "codex-bot-auth-1", exact: true }).check();
  ...
  await dialog.getByRole("radio", { name: "codex-bot-auth-3", exact: true }).check();
  await dialog.getByRole("button", { name: "Assign", exact: true }).click();
  await dialog.waitFor({ state: "hidden" });
  await card.getByRole("button", { name: "Start…", exact: true }).click();
  assert.equal(await dialog.getByRole("radio", { name: "codex-bot-auth-3", exact: true }).isChecked(), true);
  ...
  await usageWindow.getByRole("meter", { name: "codex-bot-auth-1 5 hours remaining", exact: true }).waitFor();
  assert.match(await usageWindow.innerText(), /\$300\.00 included/);
  await usageWindow.getByRole("button", { name: "Inspect grok-bot-auth-1 usage", exact: true }).waitFor();
  await usageWindow.getByRole("button", { name: "Inspect grok-worker-auth-1 usage", exact: true }).click();
  ...
  await jump("model catalog codex-worker-auth-1", "codex-worker-auth-1 models");
  ...
  await models.getByRole("tab", { name: /devin-worker-auth-1/ }).click();
