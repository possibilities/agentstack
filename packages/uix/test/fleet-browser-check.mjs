...
try {
  ...
  await dialog.getByRole("radio", { name: "codex-1", exact: true }).check();
  ...
  await dialog.getByRole("radio", { name: "codex-3", exact: true }).check();
  await dialog.getByRole("button", { name: "Assign", exact: true }).click();
  await dialog.waitFor({ state: "hidden" });
  await card.getByRole("button", { name: "Start…", exact: true }).click();
  assert.equal(await dialog.getByRole("radio", { name: "codex-3", exact: true }).isChecked(), true);
