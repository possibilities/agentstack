...
export type Owner = Pick<OwnerStatus, "uixUrl" | "inspectorUrl" | "mcpUrls"> & { children: Child[] };
...
export function indexData(owner: Owner | null, allBots: Bot[] | null): IndexData {
  ...
  const links = owner ? [
    ...
    { name: "Package API reference", url: owner.uixUrl ? new URL("/x/fleet?reference=overview", owner.uixUrl).href : null },
