...
import type { Account, Bot, BotSettings, Login, OwnerStatus, PackageDoc, Resource, Snapshot, VoiceCall, WorkerAccount, WorkerRuntime } from "./types";
...
export async function loadSnapshot(): Promise<Snapshot> {
  const [owner, accounts, workerAccounts, workerRuntimes, login, bots, botDefaults, voice, catalog] = await Promise.all([
  ...
  return { owner, accounts, workerAccounts, workerRuntimes, login, bots, botDefaults, voice, catalog, endpoints: websocketEndpoints(catalog.data) };
}
