...
Fx is also built twice from isolated Zig caches in the Linux x86-64 job and compared before staging. The manifest records the Linux digest as a release candidate and public run `35537630615` as one observation. A different repeated Darwin arm64 digest is development evidence; the difference's cause is unknown. Qualification remains pending until the repeated Linux builds match each other and the candidate.
