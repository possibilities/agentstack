...
`agentstack serve` starts the required `api`, `auth`, `bots`, `brain`, `roles`, `workers`, `usage`, `infer`, and `wiki` socket children and the WebSocket, Inspector, and UI canvas children. It serves `owner` status on its own socket and hosts the configured MCP Package APIs in the owner process. Each Package API is a line-delimited JSON socket under `<state>/sockets/<name>.sock`. The MCP URLs default to `http://127.0.0.1:8743/mcp/{auth,bots,brain,owner,roles,usage,wiki,workers}`; set `AGENTSTACK_MCP_PORT` before starting to change the port (or use `0` to allocate one and read the printed URLs). MCP exposes operations and owner-managed event subscription tools; socket and WebSocket event subscriptions remain separate.

Brain starts with an empty research index under `<state>/brain`. The AgentStack Chrome and Android clients share links and text through its authenticated device listener, loopback port `8877` by default. See [Brain and device sharing](brain.md) for storage, ingestion behavior and client configuration.
