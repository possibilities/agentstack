...
Codex is the first server: three operations, unix socket only, at `<state>/sockets/codex.sock`. MCP and websocket can be written down before they run. Codex no longer speaks MCP. Its package UI still reads loopback `/ui-data` from the socket process.
